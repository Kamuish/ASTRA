...
class HARPSN(ESO_PIPELINE):
    ...
    *Note:* Check the **User parameters** of the parent classes for further customization options of SBART
    ...
    def __init__(
        ...
        if (
            user_configs.get("use_old_pipeline", False)
            and not search_status.is_good_flag
        ):
    ...
    def _load_old_DRS_KWs(self, header):
        ...
        self.observation_info["MAX_BERV"] = (
            header["HIERARCH TNG DRS BERVMX"] * kilometer_second
        )
        self.observation_info["BERV"] = (
            header["HIERARCH TNG DRS BERV"] * kilometer_second
        )
        ...
        if bad_drift:
            logger.warning(
                "Due to previous drift-related problems, setting it to zero [m/s]"
            )
    ...
    def _load_ccf_data(self) -> None:
        ...
        self.observation_info["DRS_RV"] = (
            header["HIERARCH TNG DRS CCF RV"] * kilometer_second
        )
    ...
    def build_HARPS_wavelengths(self, hdr):
        ...
        A = np.reshape(
            [
                hdr["HIERARCH TNG DRS CAL TH COEFF LL" + str(i)]
                for i in range(omax * (d + 1))
            ],
            (omax, d + 1),
        )  # slow 30 ms
    ...
    def find_files(self, file_name: UI_PATH):
        ...
        if file_name.is_dir():
            ...
            logger.debug(
                "Received path of E2DS file; searching for CCF with matching name"
            )
            ...
            if found_CCF:
                ...
                logger.critical(
                    "Was not able to find CCF file. Marking frame as invalid"
                )
                ccf_path = ""
