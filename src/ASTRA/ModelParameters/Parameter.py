...
class ModelComponent(BASE):
    ...
    def __init__(
        ...
        """Define a parameter in our RV model.

        Contains information regarding the initial guess and the bounds that the parameter can take
        ...
        name: str
            parameter name
        initial_guess: Any
            default guess for the parameters. Unless a children overrides the generate_priors function,
            this will be always used for all frameIDs
        bounds: List[Any, ANy]
    ...
    def generate_priors(self, DataClassProxy):
        """Generate the initial guess and parameter bounds.

        Currently, only supports using the same initial guess
