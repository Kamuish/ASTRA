...
from ASTRA.utils.choices import DISK_SAVE_MODE, TELLURIC_APPLICATION_MODE, TELLURIC_EXTENSION, WORKING_MODE
from ASTRA.utils.create_spectral_blocks import build_blocks
from ASTRA.utils.custom_exceptions import NoDataError
from ASTRA.utils.parameter_validators import BooleanValue, ValueInInterval
from ASTRA.utils.shift_spectra import (
    apply_approximated_BERV_correction,
    apply_BERV_correction,
)
from ASTRA.utils.spectral_conditions import Empty_condition, KEYWORD_condition
from ASTRA.utils.telluric_utilities import create_binary_template
from ASTRA.utils.telluric_utilities.compute_overlaps_blocks import find_overlaps
...
class TelluricTemplate(BaseTemplate):
    ...
    *Note:* Also check the **User parameters** of the parent classes for further customization options of SBART
    ...
    def load_information_from_DataClass(self, dataClass: DataClass) -> None:
        """Load the necessary information from the dataClass to generate the models.
    ...
    def _load_BERV_info(self, DataClass) -> None:
        """Load the BERV and MaxBERV of all observations from the subInstrument associated with this template.
    ...
    def ingest_new_rolling_observations(self, dataClass):
        """Ingest new observations into a rolling mode template."""
        if self.work_mode != WORKING_MODE.ROLLING:
    ...
    def _extend_detections(self, telluric_block: list[list]) -> list[list[float]]:
        ...
        Independently of the extension mode, the telluric features are widened by +- 15m/s

        Args:
            telluric_block (list[list]): Start and end of a wavelength block that has a telluric feature!

        Returns:
            list[list[float]]: Updated position of the feature
    ...
    def create_binary_template(self, continuum_level) -> None:
        """Transform transmittance spectra to binary template, based on percentage deviations from the continuum level.
