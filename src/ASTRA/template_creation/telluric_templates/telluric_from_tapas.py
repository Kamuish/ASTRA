...
from ASTRAutils import custom_exceptions, get_TAPAS_data
from ASTRAutils.shift_spectra import remove_BERV_correction
    ...
    **User parameters:**
    ...
    def create_telluric_template(self, dataClass, custom_frameID: Optional[int] = None) -> None:
        ...
        continuum_level[0 : n_points_filter + 1] = median_filter(
            self.transmittance_spectra[0 : n_points_filter + 1],
            51,
        )
        continuum_level[-(n_points_filter + 1) :] = median_filter(
            self.transmittance_spectra[-(n_points_filter + 1) :],
            51,
        )
