...
class Frame(Spectrum, Spectral_Modelling, Spectral_Normalization):
    ...
    _default_params = DefaultValues(
        ...
        bypass_ST_designation=UserParam(
            default_value=None, constraint=ValueFromIterable((None, "S2D", "S1D"))
        ),
    ...
    def reject_wavelength_region_from_order(
        self, order: int, region: list[tuple[int, int]]
    ) -> None:
    ...
    def mark_wavelength_region(
        self, reason: Flag, wavelength_blocks: list[tuple[int, int]]
    ) -> None:
    ...
    def select_wavelength_region(
        self, order: int, wavelength_blocks: list[tuple[int, int]]
    ) -> None:
    ...
    def finalized_external_data_load(self) -> None:
        ...
        if not self.is_valid:
            logger.warning(
                "Finalizing external data loading for Frame that was already rejected."
            )
        else:
            self._status.delete_flag(LOADING_EXTERNAL_DATA)
    ...
    def build_mask(
        ...
        self.spectral_mask.add_indexes_to_mask(
            np.where(~np.isfinite(self.spectra)), MISSING_DATA
        )
        self.spectral_mask.add_indexes_to_mask(
        ...
        self.spectral_mask.add_indexes_to_mask(
            np.where(~np.isfinite(self.uncertainties)), MISSING_DATA
        )
        ...
        if self._internal_configs["SIGMA_CLIP_FLUX_VALUES"] > 0:
            ...
            threshold = (
                ...
                + median_level[:, np.newaxis]
            )
            self.spectral_mask.add_indexes_to_mask(
                np.where(self.spectra > threshold), MISSING_DATA
            )
    ...
    def assess_bad_orders(self) -> None:
        ...
        if (
            len(self.bad_orders)
            >= self._internal_configs["MAX_ORDER_REJECTION"] * self.N_orders / 100
        ):
            logger.warning(
    ...
    def find_instrument_type(self) -> None:
        ...
        for key, threshold in self.__class__.sub_instruments.items():
            ...
            raise custom_exceptions.InternalError(
                "no sub-instrument found for observation"
            )
