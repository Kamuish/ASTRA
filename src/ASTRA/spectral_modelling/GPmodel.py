...
import numpy as np
from ASTRA.ModelParameters.Parameter import JaxComponent
...
from ASTRA.status.flags import INTERNAL_ERROR, SUCCESS
...
class GPSpecModel(ModellingBase):
    ...
    *Note:* Also check the **User parameters** of the parent classes for further customization options of SBART
    ...
    _default_params = ModellingBase._default_params + DefaultValues(
        GP_KERNEL=UserParam(
            "Matern-5_2", constraint=ValueFromIterable(["Matern-5_2", "Matern-3_2"])
        ),
        FORCE_MODEL_GENERATION=UserParam(False, constraint=BooleanValue),
        POSTERIOR_CHARACTERIZATION=UserParam(
            "minimize", constraint=ValueFromIterable(["minimize", "MCMC"])
        ),
    ...
    def __init__(self, obj_info, user_configs):
        ...
        for kern in self._default_params[
            ...
            possible_folders[f"modelling_information_{kern}"] = (
